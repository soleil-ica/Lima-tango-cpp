...
void LimaDetector::write_acquisitionMode(Tango::WAttribute &attr)
{
	...
	try
	{
		m_acquisition_mode = *attr_acquisitionMode_read;//memorize previous valid value
		...
		{
			delete[] attr_acquisitionMode_write;
		...
		//- current IS AN AVAILABLE ACQUISITION MODE
		if (current == "SINGLE")
		{
		...
		if (current == m_acquisition_mode)//if acquisition mode is the same than the previous one -> no need to recreate again image dynmaic attribute
...
void LimaDetector::write_flipX(Tango::WAttribute &attr)
{
	...
	try
	{
		...
		PropertyHelper::set_property(this, "MemorizedFlipX", attr_flipX_write);
	}
	catch (Tango::DevFailed& df)
	{
		ERROR_STREAM << df << endl;
...
void LimaDetector::write_flipY(Tango::WAttribute &attr)
{
	...
	try
	{
		...
		if (attr_flipY_write == true)
			...
			flip.y = false;
		m_ct->image()->setFlip(flip);
		PropertyHelper::set_property(this, "MemorizedFlipY", attr_flipY_write);
	}
	catch (Tango::DevFailed& df)
	{
		ERROR_STREAM << df << endl;
