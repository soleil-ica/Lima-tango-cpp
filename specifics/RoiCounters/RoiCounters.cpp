...
void RoiCounters::update_roi()
{
	...
	try
	{
		...
		for (std::list<SoftOpRoiCounter::RoiNameAndRoi>::iterator iter = listNameAndRois.begin();
		 ...
		 iter++)
		{
			INFO_STREAM << (*iter).first << "\t : " << (*iter).second << endl;
		}
...
void RoiCounters::read_roi()
{
	...
	try
	{
		if ((reinterpret_cast<SoftOpRoiCounter*> (m_soft_operation.m_opt)) != 0)
		{
			...
			INFO_STREAM << "\ncounter_status = " << ((reinterpret_cast<SoftOpRoiCounter*> (m_soft_operation.m_opt))->getCounterStatus() + 1) << endl;
...
void RoiCounters::read_rois_callback(yat4tango::DynamicAttributeReadCallbackData& cbd)
{
	...
	try
	{
		...
		if (std::size_t pos = attrName.find("x") != std::string::npos)
		{
			...
			if (std::size_t pos = attrName.find("y") != std::string::npos)
		{
			std::string strIndex = attrName.substr(1);
			int attrIndex;
			std::istringstream(strIndex) >> attrIndex;
			...
			if (std::size_t pos = attrName.find("width") != std::string::npos)
		{
			...
			if (std::size_t pos = attrName.find("height") != std::string::npos)
		{
...
{
	DEBUG_STREAM << "RoiCounters::write_rois_callback(yat4tango::DynamicAttributeWriteCallbackData& cbd) - [BEGIN]" << endl;
	yat::AutoMutex<> _lock(ControlFactory::instance().get_global_mutex());
	try
	{
		std::string attrName = cbd.tga->get_name();
		...
		if (attrName.find("x") != std::string::npos)
		{
			...
			if (attrName.find("y") != std::string::npos)
		{
			std::string strIndex = attrName.substr(1);
			std::istringstream(strIndex) >> attrIndex;
			val = (Tango::DevULong *) & attr_y_array[attrIndex];
			...
			if (attrName.find("width") != std::string::npos)
		{
			std::string strIndex = attrName.substr(5);
			std::istringstream(strIndex) >> attrIndex;
			val = (Tango::DevULong *) & attr_width_array[attrIndex];
			...
			if (attrName.find("height") != std::string::npos)
		{
			std::string strIndex = attrName.substr(6);
			std::istringstream(strIndex) >> attrIndex;
			val = (Tango::DevULong *) & attr_height_array[attrIndex];
		...
		INFO_STREAM << "attr_width_array[attrIndex] = " << attr_width_array[attrIndex] << endl;
		INFO_STREAM << "attr_height_array[attrIndex] = " << attr_height_array[attrIndex] << endl;
	...
	DEBUG_STREAM << "RoiCounters::write_rois_callback(yat4tango::DynamicAttributeWriteCallbackData& cbd) - [END]" << endl;
}
